...
async def on_command_error(ctx, error) :
    if type(error) == commands.errors.CommandOnCooldown : await on_command_on_cooldown(ctx, error)
    if type(error) == commands.errors.CommandNotFound : await on_command_not_found(ctx, error)
